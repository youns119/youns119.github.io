...
각 층과 각 방들을 하나의 배열의 요소로 생각해 `2차원 배열`을 만들고, 모든 방들의 값들을 `누적합`으로 구해서 미리 테이블을 만들어 놓았다. 그렇게 해서 **<u>입력이 들어올 때마다 바로 배열에 임의 접근으로 값을 출력</u>**할 수 있었다. 
...
나의 `2차원 배열`을 사용하는 방법에 비해 메모리를 절반으로 아낄 수 있다. 그 대신 입력이 들어올 때마다 다시 계산을 해야해서 **<u>테스트 케이스의 수가 커지거나, k, n의 값이 커질 경우 속도가 확 떨어질 수 있다는 단점도 존재</u>**한다. 
