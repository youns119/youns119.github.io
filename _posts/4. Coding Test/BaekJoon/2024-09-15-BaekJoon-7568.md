...
다른 더 좋은 풀이가 있나 생각했는데, **<u>모든 루프를 이중으로 돌더라도 '2 ≤ N ≤ 50' 조건에서 'O(N^2)'면 기껏해야 '2500번' 루프를 돌게 되므로, '1초'의 시간 제한에서는 매우 널널하다.</u>** 

따라서 그냥 `vector`에 입력값들을 모두 저장하고 나중에 `브루트 포스` 방식으로 이중 반복문을 돌면서 값을 구하도록 구현했다.
