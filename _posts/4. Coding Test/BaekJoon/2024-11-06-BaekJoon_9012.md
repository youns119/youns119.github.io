...
사실상 이 문제의 본질은 `스택`을 사용하는 것이나, 그냥 `문자열`만으로 풀 수 있다. `문자열` 내의 문자들을 순회하면서 괄호의 열림, 닫힘을 체크하는 방식으로 하면 간단하다. 그리고 그 코드는 깔끔하게 `함수` 하나로 뺐다.
...
이 문제의 의도대로, 가장 정석인 `스택`을 이용한 풀이이다. **<u>괄호의 종류에 따라 '스택'에 넣거나 빼거나 하면서 문자열을 검사하는 방식</u>**이다. 

코드 자체는 위의 풀이와 크게 다르지 않지만, `스택`이라는 컨테이너를 하나 더 쓰는 것 뿐이다.
...
사실상 정석은 두 번째 풀이인 `스택`을 이용하는 것이겠지만, 첫 번째 풀이가 더 간단하다. 게다가 `stack` 컨테이너를 쓰지 않기 때문에 **<u>헤더 파일 포함도 안해도 되고</u>**, `문자열` 자체에서 모든 로직을 구현 가능하기 때문에 아주 미세하지만 **<u>성능도 첫 번째 풀이가 좀 더 좋다.</u>**