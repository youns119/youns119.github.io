...
`N` 범위 내의 모든 루프를 돌며 자릿수 체크와 덧셈/뺌셈 연산을 하므로 시간 복잡도는 `O(N)`이라고 할 수 있다. 거기다 안쪽 반복문도 지금 내 구현 상으로는 `O(1)`이지만 `pow`라는 비싼 연산도 있고 상수 시간으로 쳐도 꽤나 비싼 연산이다.
...
1. `int` 자료형을 `string`으로 변환하고 문자열 길이를 통해 자릿수를 구한다.
2. 각 자릿수가 9일 때 자릿수의 합이 최대이므로 `iN - 9 * iDigit`이 최솟값이 된다.
3. 현재 숫자에 `mod` 연산, 나눗셈 연산을 통해 적은 반복으로 각 자릿수 더한다.

이런 방식으로 시간 복잡도를 `O((log N)²)` 까지 획기적으로 줄일 수 있고, 이는 거의 상수 시간으로 봐도 될 정도로 빠르다.
