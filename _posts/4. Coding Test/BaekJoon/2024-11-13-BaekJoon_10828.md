...
문제의 본래 의도는 `stack`이지만, 대신 `vector`를 사용하는 풀이이다. `stack`의 모든 기능은 `vector`로 가능하므로, **<u>완전 치환 가능</u>**하다.
...
문제의 본래 의도대로 `stack`을 사용하는 풀이이다. 로직은 위의 풀이와 완전히 같고, 컨테이너만 `stack`으로 치환했다.
...
`stack` 문제는 모두 `vector`로 치환 가능하다. 게다가 `vector`를 사용하면 **<u>'reserve'로 재할당 방지, 연속 메모리 구조로 인한 높은 '캐시 친화성'이라는 장점</u>**까지 있으므로 대부분의 경우에는 `vector`를 사용하는 편이 좋다.